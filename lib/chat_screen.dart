...
class ChatScreenState extends State<ChatScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _startBubbleTimer(); // Appel de la méthode pour démarrer le minuteur de la bulle de discussion
  ...
  void _handleSubmitted(String text) async {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (definitionsElement != null) {
            ...
            // Supprime les retours à la ligne entre les mots
            definitionsText = definitionsText.replaceAll(RegExp(r'\s+'), ' ');

          // Met le texte en taille 16
            Text(
              definitionsText,
              style: TextStyle(
                fontSize: 16.0,
    ...
    ChatMessage aiMessage = ChatMessage(
            ...
            await Future.delayed(Duration(seconds: 4)); // Attendre quelques secondes avant d'afficher la définition suivante
          }
        } else {
  ...
  void _startBubbleTimer() {
    _bubbleTimer = Timer.periodic(Duration(seconds: 15), (timer) {
      _showBubbleMessage();
    });
  }
