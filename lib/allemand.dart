...
class allemand extends StatefulWidget {
  final String chatName;
  final int index;
  final bool isDarkModeEnabled;
  allemand({required this.chatName, required this.index, required this.isDarkModeEnabled});

  @override
  allemandState createState() => allemandState();
}

class allemandState extends State<allemand> with SingleTickerProviderStateMixin {
  // Liste des messages
  ...
  Future<void> initializeData() async {
    ...
    String welcomeMessage = "Bienvenue ! 😁\nJe suis là pour vous aider à trouver les définitions des mots en français-allemand. N'hésitez pas à poser vos questions.";
  ...
  void _handleSubmitted(String text) async {
    ...
    String url = isFrancaisItalien
        ? "https://www.larousse.fr/dictionnaires/francais-allemand/$text"
        : "https://www.larousse.fr/dictionnaires/allemand-francais/$text";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (definitionsElement != null) {
          ...
          String errorMessageAll = "Entschuldigung, aber ich verstehe Ihr Wort nicht 😅. \n -Bitte überprüfen Sie es, wenn möglich, da ich viel effektiver bin, wenn die Wörter einzeln eingegeben werden. \n -Es könnte auch ein Problem mit meinen Diensten vorliegen. In diesem Fall bitten wir um Geduld.";
          ChatMessage aiMessage = ChatMessage(
            text: isFrancaisItalien ? errorMessage : errorMessageAll,
            isUserMessage: false,
            time: DateTime.now(),
      ...
      String errorMessageAll = "Entschuldigung 😥, bitte überprüfen Sie Ihre Internetverbindung.";
      ChatMessage aiMessage = ChatMessage(
        text: isFrancaisItalien ? errorMessage : errorMessageAll,
        isUserMessage: false,
        time: DateTime.now(),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
            CircleAvatar(
              backgroundImage: AssetImage('images/imana.jpg'),
            ),
            SizedBox(width: 8.0),
            Text(isFrancaisItalien ? 'Français - Allem..' : 'Deutsch - Franzö..',
            ...
            itemBuilder: (BuildContext context) => [
              PopupMenuItem(
                child: ListTile(
                  ...
                  title: Text( isFrancaisItalien ? 'Historique des discussions' : 'Verlauf der Unterhaltungen',
                    style: TextStyle(
                        color: Colors.white),
                  ),
                  onTap: () async {
              ...
              PopupMenuItem(
                child: ListTile(
                  ...
                  title: Text(isFrancaisItalien ? 'Französisch - Deutsch' : 'Allemand - Français',
                    style: TextStyle(
                        color: Colors.white),
                  ),
                  onTap: () {
                    ...
                    setState(() {
                      _messages[0] = ChatMessage(
                        text: "Super ! 😁 Nous passons à nouveau au français. \nje vais vous aider à trouver les définitions des mots en français-allemand. \nN'hésitez pas à poser vos questions.",
                        isUserMessage: false,
                        time: DateTime.now(),
                        soundUrl: "",
                      );
                      ...
                      _messages[0] = ChatMessage(
                        text: "Super ! 😁 Nous passons à présent à l'allemand. \nIch werde Ihnen helfen, die Definitionen der Wörter in Deutsch-Französisch zu finden. \nZögern Sie nicht, Ihre Fragen zu stellen.",
                        isUserMessage: false,
                        time: DateTime.now(),
                        soundUrl: "",
                      );
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: isFrancaisItalien ? 'Ecrire un mot en français' : 'Schrift ein Wort auf Deutsch.',
                              ),
                            ),
                          ),
                          IconButton(
  ...
  Widget build(BuildContext context) {
    ...
    final messageBubble = GestureDetector(
      ...
      child: Container(
        padding: EdgeInsets.all(8.0),
        decoration: BoxDecoration(
          color: isUserMessage ? Colors.white : Colors.purple,
          borderRadius: BorderRadius.circular(16.0),
          boxShadow: [
            BoxShadow(
              color: Colors.black.withOpacity(0.2),
